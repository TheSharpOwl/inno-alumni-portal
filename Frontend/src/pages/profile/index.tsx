...
const fetchUserProfile = async (token) => {
  try {
    const response = await axios.get(`${apiEndPoint}/accounts/profile`, {
...
const ProfilePicture = ({ src, alt }) => {
  return (
    <div className="mr-4">
      {src ? (
        <Avatar src={src} alt={alt} size={48} />
      ) : (
        <div className="text-[#40BA21]">
          <HiUserCircle size={96} />
        </div>
      )}
    </div>
  );
...
const EditButtons = ({ onEdit, onChangePassword }) => {
  return (
    <div>
      <div>
        <button
      ...
      <div>
        <button className="mt-4 hover:text-[#40BA21] underline" onClick={onChangePassword}>
          Change Password
        </button>
      </div>
    </div>
